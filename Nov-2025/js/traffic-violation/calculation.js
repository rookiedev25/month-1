...
const userMessage = (distance, vehicles, locationSpeedLimit) => {
    vehicles.forEach((item) => {
        ...
        const violationStatus = hasViolated(locationSpeedLimit, speed, item.location)
        const fine = calculateFine(item.type, violationStatus)
        if (fine) {
            console.log(`Dear user, your ${item.type} was found passing our speed cameras at ${item.location} with a speed of ${speed.toFixed(2)}. Please pay the fine amount: ${fine}`)
        }
        else {
